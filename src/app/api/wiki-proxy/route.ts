...
function processHtml(html: string, currentPage: string): string {
  ...
  const trackingScript = `
    <script>
      ...
      document.addEventListener('click', function(e) {
        ...
        if (link && link.href) {
          ...
          if (href.includes('/wiki/')) {
            ...
            window.location.href = '/api/wiki-proxy?page=' + wikiPage;
      ...
      window.addEventListener('DOMContentLoaded', function() {
        ...
        // Notify parent that the page is fully loaded
        window.parent.postMessage({ 
          type: 'WIKI_PAGE_LOADED'
        }, '*');
      });

      // Also notify when all resources are loaded
  ...
  return (
    html
      ...
      // Wiki links
      .replace(/href="\/wiki\//g, `href="/api/wiki-proxy?page=`)
      // CSS links
